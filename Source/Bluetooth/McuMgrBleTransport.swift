...
public protocol EAPeripheralDelegate: class {
    /// Callback called whenever peripheral state changes.
    func peripheral(_ peripheral: EAAccessory, didChangeStateTo state: PeripheralState)
}

public class McuMgrBleTransport: NSObject, EAAccessoryDelegate, StreamDelegate {
 ...
 //   private let centralManager: CBCentralManager
    ...
    private let EAcentralManager: EAAccessoryManager
  ...
  /*  public weak var delegate: EAAccessoryDelegate? {
        didSet {
    ...
    ///   Protocol (SMP) service.
       //public convenience init(_ target: EAAccessory) {
       //     self.init(target)
       // }
    ...
    ///   Protocol (SMP) service.
    //
    public init(target: EAAccessory) {
       // self.centralManager = CBCentralManager(delegate: nil, queue: nil) //委托和队列
        
        self._session = EASession(accessory: target, forProtocol: target.protocolStrings[0])
        self.dispatchQueue = DispatchQueue(label: "McuMgrBleTransport")
        self.lock = ResultLock(isOpen: false)
        self.observers = []
     //   self.EAperipheral = EAperipheral
        self.EAcentralManager = EAAccessoryManager.shared()
        self.operationQueue = OperationQueue()
        self.operationQueue.maxConcurrentOperationCount = 1
        
        self.sessionController = SessionController.sharedController
        ...
        if let _accessory = EAcentralManager.connectedAccessories.first {
            self._accessory = _accessory
        }
...
/*    private func notifyPeripheralDelegate() {
        if let EAperipheral = self.EAperipheral {
            delegate?.peripheral(EAperipheral, didChangeStateTo: state)
        }
    }*/
}
...
extension McuMgrBleTransport: McuMgrTransport {
    ...
    public func send<T: McuMgrResponse>(data: Data, callback: @escaping McuMgrCallback<T>) {
        dispatchQueue.async {
            self.operationQueue.addOperation {
                self._send(data: data, callback: callback)
            }
        }
    }
    
    private func updateReadData() {
        let bufferSize = 1024
        var buffer = [UInt8](repeating: 0, count: bufferSize)
        responseData = Data()
        while _session!.inputStream!.hasBytesAvailable == true {
            var result = _session!.inputStream!.read(&buffer, maxLength: buffer.count)
            responseData.append(contentsOf: buffer[0..<result])
        }
    ...
    private func writeData() {    //写数据
        print("Lock", _session!.outputStream!.hasSpaceAvailable, _writeData)
        if (_session?.outputStream!.hasSpaceAvailable == false) || (_writeData.length == 0) || (self._writeLock.try() == false) {
            return
        }
        ...
        while (_session?.outputStream?.hasSpaceAvailable)! == true && (_writeData.length) > 0 {
            ...
            _writeData.getBytes(&buffer, length: (_writeData.length))  //_writeData是写的对象
            let bytesWritten = _session!.outputStream!.write(&buffer, maxLength: _writeData.length)
            print("write------", bytesWritten)
            if bytesWritten == -1 {
                print("Write Error")
                break
            } else if bytesWritten > 0 {
                print("1")
                var count = min(bytesWritten, _writeData.count)
                _writeData.replaceBytes(in: NSMakeRange(0, count), withBytes: nil, length: 0)
                print("2", _session!.outputStream!.hasSpaceAvailable, _writeData)
            }
        }
        
        print("UnLock")
        self._writeLock.unlock()
        print("UnLocked", _session!.outputStream!.hasSpaceAvailable, _writeData)
    }
    
    
    
    private func _send<T: McuMgrResponse>(data: Data, callback: @escaping McuMgrCallback<T>) -> Bool {
       // _writeData?.replaceBytes(in: NSMakeRange(0, 20000), withBytes: nil, length: 0)
        //print(">>>>>:", self._writeData, data)
        self._writeData.append(data)
        //dispatchQueue.async {
            self.writeData()
        
        //}
        //print("-----:", self._writeData)
        
        let result = lock.block(timeout: DispatchTime.now() + .seconds(10))
        //print("*****:", self._writeData)
        switch result {
        case .timeout:
        ...
        case .success:
            do {
                ...
                log(msg: "<- \(responseData.hexEncodedString(options: .prepend0x) )",
                    atLevel: .debug)
    ...
    public func EAClose() {
        if let EAperipheral = _accessory, EAperipheral.isConnected != false{
            EAcentralManager.unregisterForLocalNotifications()
        }
    }
   ...
   /* private func _send1<T: McuMgrResponse>(data: Data, callback: @escaping McuMgrCallback<T>) -> Bool {
        if let EAperipheral = EAperipheral, EAperipheral.isConnected == false{
            fail(error: McuMgrBleTransportError.centralManagerPoweredOff, callback: callback)
            return false
        }
        
        let target: EAAccessory
        
        if let existing = EAperipheral, EAperipheral!.isConnected == true{
            target = existing
        }else
        {
